...
public final class JBGBattleUtils {
  ...
  public static boolean checkForOverwhelmingWin(
    ...
    final double power = estimatePower(jbgData, t, defendingUnits, attackingUnits, false);
  ...
  public static double estimateStrengthDifference(
    ...
    if (attackingUnits.stream().allMatch(Matches.unitIsInfrastructure())
        || estimatePower(jbgData, t, attackingUnits, defendingUnits, true) <= 0) {
      return 0;
    }
    if (defendingUnits.stream().allMatch(Matches.unitIsInfrastructure())
        || estimatePower(jbgData, t, defendingUnits, attackingUnits, false) <= 0) {
      return 99999;
    }
    final double attackerStrength =
        estimateStrength(jbgData, t, attackingUnits, defendingUnits, true);
    final double defenderStrength =
        estimateStrength(jbgData, t, defendingUnits, attackingUnits, false);
  ...
  public static double estimateStrength(
    ...
    final double myPower = estimatePower(jbgData, t, myUnits, enemyUnits, attacking);
  ...
  private static double estimatePower(
    ...
    sortedUnitsList.sort(
        new UnitBattleComparator(
                !attacking, jbgData.getUnitValueMap(), TerritoryEffectHelper.getEffects(t), data)
            .reversed());
    final int myPower =
        TotalPowerAndTotalRolls.getTotalPower(
            TotalPowerAndTotalRolls.getUnitPowerAndRollsForNormalBattles(
  ...
  public static boolean territoryHasLocalLandSuperiority(
      final JBGData jbgData, final Territory t, final int distance, final GamePlayer player) {
    return territoryHasLocalLandSuperiority(jbgData, t, distance, player, new HashMap<>());
  }
  ...
  public static boolean territoryHasLocalLandSuperiority(
    ...
    for (int i = 2; i <= distance; i++) {
      ...
      final Set<Territory> nearbyTerritoriesForEnemy =
          data.getMap()
              .getNeighbors(t, i, JBGMatches.territoryCanMoveLandUnits(player, data, false));
      ...
      for (final Territory nearbyTerritory : nearbyTerritoriesForEnemy) {
        enemyUnits.addAll(
            nearbyTerritory
                .getUnitCollection()
                .getMatches(JBGMatches.unitIsEnemyNotNeutral(player, data)));
      }

      // Find allied strength
      final Set<Territory> nearbyTerritoriesForAllied =
          data.getMap()
              .getNeighbors(t, i - 1, JBGMatches.territoryCanMoveLandUnits(player, data, false));
      ...
      final double strengthDifference =
          estimateStrengthDifference(jbgData, t, enemyUnits, alliedUnits);
      JBGLogger.trace(
          t
  ...
  public static boolean territoryHasLocalLandSuperiorityAfterMoves(
    ...
    final Set<Territory> nearbyTerritoriesForEnemy =
        data.getMap()
            .getNeighbors(t, distance, JBGMatches.territoryCanMoveLandUnits(player, data, false));
    nearbyTerritoriesForEnemy.add(t);
    final List<Unit> enemyUnits = new ArrayList<>();
    for (final Territory nearbyTerritory : nearbyTerritoriesForEnemy) {
      enemyUnits.addAll(
    ...
    final Set<Territory> nearbyTerritoriesForAllied =
        data.getMap()
            .getNeighbors(
                t, distance - 1, JBGMatches.territoryCanMoveLandUnits(player, data, false));
    nearbyTerritoriesForAllied.add(t);
    ...
    for (final Entry<Territory, JBGTerritory> entry : moveMap.entrySet()) {
    ...
    // Determine strength difference
    final double strengthDifference =
        estimateStrengthDifference(jbgData, t, enemyUnits, new ArrayList<>(alliedUnits));
    JBGLogger.trace(
        t
            + ", current enemy land strengthDifference="
            + strengthDifference
  ...
  public static boolean territoryHasLocalNavalSuperiority(
    ...
    for (final Territory nearbySeaTerritory : nearbyAlliedSeaTerritories) {
      myUnitsInSeaTerritories.addAll(
          nearbySeaTerritory.getUnitCollection().getMatches(JBGMatches.unitIsOwnedNotLand(player)));
      myUnitsInSeaTerritories.addAll(
    ...
    final double defenseStrengthDifference =
        estimateStrengthDifference(jbgData, t, enemyAttackers, myUnits);
    JBGLogger.trace(
        t
    ...
    double attackStrengthDifference =
        estimateStrengthDifference(jbgData, t, myUnits, enemyUnitsInSeaTerritories);
    attackStrengthDifference +=
        0.5
            * estimateStrengthDifference(
                jbgData, t, alliedUnitsInSeaTerritories, enemyUnitsInSeaTerritories);
    JBGLogger.trace(
        t
