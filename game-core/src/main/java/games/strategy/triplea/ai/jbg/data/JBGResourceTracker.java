...
public class JBGResourceTracker {

  private final IntegerMap<Resource> resources;
  private IntegerMap<Resource> tempPurchases = new IntegerMap<>();

  public JBGResourceTracker(final GamePlayer player) {
    resources = player.getResources().getResourcesCopy();
  }

  public JBGResourceTracker(final int pus, final GameData data) {
    resources = new IntegerMap<>();
    resources.add(data.getResourceList().getResource(Constants.PUS), pus);
  }

  public boolean hasEnough(final JBGPurchaseOption ppo) {
    return getRemaining().greaterThanOrEqualTo(ppo.getCosts());
  }

  public void purchase(final JBGPurchaseOption ppo) {
    resources.subtract(ppo.getCosts());
  }

  public void removePurchase(final JBGPurchaseOption ppo) {
    if (ppo != null) {
      resources.add(ppo.getCosts());
    }
  }

  public void tempPurchase(final JBGPurchaseOption ppo) {
    tempPurchases.add(ppo.getCosts());
  }

  public void removeTempPurchase(final JBGPurchaseOption ppo) {
    if (ppo != null) {
