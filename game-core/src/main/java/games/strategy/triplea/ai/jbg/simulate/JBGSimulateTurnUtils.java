...
/** JBG AI simulate turn utilities. */
@SuppressWarnings("deprecation") //JBG: disable Observable warning
public final class JBGSimulateTurnUtils {
  private JBGSimulateTurnUtils() {}
  ...
  public static void simulateBattles(
      final JBGData proData,
      final GameData data,
      final GamePlayer player,
      final IDelegateBridge delegateBridge,
      final JBGOddsCalculator calc) {

    JBGLogger.info("Starting battle simulation phase");
    ...
    for (final Entry<BattleType, Collection<Territory>> entry : battleTerritories.entrySet()) {
      for (final Territory t : entry.getValue()) {
        ...
        JBGLogger.debug("---" + t);
        JBGLogger.debug("attackers=" + attackers);
        JBGLogger.debug("defenders=" + defenders);
        JBGLogger.debug("bombardingUnits=" + bombardingUnits);
        ...
        JBGLogger.debug("remainingAttackers=" + remainingAttackers);
        JBGLogger.debug("remainingDefenders=" + remainingDefenders);
        ...
        JBGLogger.debug("attackersToRemove=" + attackersToRemove);
        JBGLogger.debug("defendersToRemove=" + defendersToRemove);
        JBGLogger.debug("infrastructureToChangeOwner=" + infrastructureToChangeOwner);
   ...
   */
  public static Map<Territory, JBGTerritory> transferMoveMap(
      final JBGData proData,
      final Map<Territory, JBGTerritory> moveMap,
      final GameData toData,
      final GamePlayer player) {

    JBGLogger.info("Transferring move map");
    ...
    for (final Territory fromTerritory : moveMap.keySet()) {
      ...
      final JBGTerritory patd = new JBGTerritory(toTerritory, proData);
      ...
      JBGLogger.debug("Transferring " + fromTerritory + " to " + toTerritory);
      ...
      for (final Unit u : moveMap.get(fromTerritory).getUnits()) {
        if (!amphibUnits.contains(u)) {
          ...
          if (toUnit != null) {
            patd.addUnit(toUnit);
            JBGLogger.trace("---Transferring unit " + u + " to " + toUnit);
          }
        }
      }
      for (final Unit u : moveMap.get(fromTerritory).getBombers()) {
        ...
        if (toUnit != null) {
          ...
          JBGLogger.trace("---Transferring bomber " + u + " to " + toUnit);
        }
      }
  ...
  private static @Nullable Unit transferUnit(
    ...
    final List<Unit> toUnits =
        toData
            ...
            .getUnitCollection()
            .getMatches(
                JBGMatches.unitIsOwnedAndMatchesTypeAndNotTransporting(player, u.getType()));
  ...
  private static @Nullable Unit transferLoadedTransport(
    ...
    final List<Unit> toTransports =
        toData
            ...
            .getUnitCollection()
            .getMatches(
                JBGMatches.unitIsOwnedAndMatchesTypeAndIsTransporting(player, transport.getType()));
