...
@ToString
public class JBGPurchaseTerritory {

  @Getter private final Territory territory;
  @Getter @Setter private int unitProduction;
  @Getter private final List<JBGPlaceTerritory> canPlaceTerritories;

  public JBGPurchaseTerritory(
  ...
  public JBGPurchaseTerritory(
    ...
    canPlaceTerritories.add(new JBGPlaceTerritory(territory));
    if (!isBid
        && JBGMatches.territoryHasFactoryAndIsNotConqueredOwnedLand(player, data).test(territory)) {
      for (final Territory t : data.getMap().getNeighbors(territory, Matches.territoryIsWater())) {
        if (Properties.getWW2V2(data)
          ...
          canPlaceTerritories.add(new JBGPlaceTerritory(t));
        }
      }
    }
  }
  ...
  public int getRemainingUnitProduction() {
    ...
    for (final JBGPlaceTerritory ppt : canPlaceTerritories) {
