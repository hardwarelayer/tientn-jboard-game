...
@Setter
@Log
final class JBGLogSettings implements Serializable {
  ...
  static JBGLogSettings loadSettings() {
    try {
      final byte[] pool =
          Preferences.userNodeForPackage(JBGAi.class).getByteArray(PROGRAM_SETTINGS, null);
      if (pool != null) {
        return IoUtils.readFromMemory(
            pool,
            is -> {
              try (ObjectInputStream ois = new ObjectInputStream(is)) {
                return (JBGLogSettings) ois.readObject();
    ...
    return new JBGLogSettings();
  }

  static void saveSettings(final JBGLogSettings settings) {
    try {
      final byte[] bytes =
      ...
      final Preferences prefs = Preferences.userNodeForPackage(JBGAi.class);
