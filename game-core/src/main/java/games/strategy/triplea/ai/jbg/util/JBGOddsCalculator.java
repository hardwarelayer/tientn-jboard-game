...
/** JBG AI odds calculator. */
public class JBGOddsCalculator {

  private final IBattleCalculator calc;
  private boolean stopped = false;

  public JBGOddsCalculator(final IBattleCalculator calc) {
    this.calc = calc;
  }
   ...
   * returned.
   */
  public JBGBattleResult estimateAttackBattleResults(
    ...
    final JBGBattleResult result =
        checkIfNoAttackersOrDefenders(jbgData, t, attackingUnits, defendingUnits, true);
    if (result != null) {
      return result;
    }
    ...
    final double strengthDifference =
        JBGBattleUtils.estimateStrengthDifference(jbgData, t, attackingUnits, defendingUnits);
    if (strengthDifference < 45) {
      return new JBGBattleResult(0, -999, false, new ArrayList<>(), defendingUnits, 1);
    }
    return callBattleCalc(jbgData, t, attackingUnits, defendingUnits, bombardingUnits);
  }
   ...
   * returned.
   */
  public JBGBattleResult estimateDefendBattleResults(
      final JBGData jbgData,
      final Territory t,
      final Collection<Unit> attackingUnits,
      final Collection<Unit> defendingUnits,
  ...
  public JBGBattleResult calculateBattleResultsNoSubmerge(
      final JBGData jbgData,
      final Territory t,
      final Collection<Unit> attackingUnits,
      final Collection<Unit> defendingUnits,
    ...
    return calculateBattleResults(
        jbgData, t, attackingUnits, defendingUnits, bombardingUnits, false);
  }

  public JBGBattleResult calculateBattleResults(
      final JBGData jbgData,
      final Territory t,
      final Collection<Unit> attackingUnits,
      final Collection<Unit> defendingUnits,
      final Collection<Unit> bombardingUnits) {
    return calculateBattleResults(
        jbgData, t, attackingUnits, defendingUnits, bombardingUnits, true);
  }

  private JBGBattleResult calculateBattleResults(
      final JBGData jbgData,
      final Territory t,
      final Collection<Unit> attackingUnits,
      final Collection<Unit> defendingUnits,
    ...
    final JBGBattleResult result =
        checkIfNoAttackersOrDefenders(jbgData, t, attackingUnits, defendingUnits, checkSubmerge);
    if (result != null) {
      return result;
    }
    return callBattleCalc(
        jbgData, t, attackingUnits, defendingUnits, bombardingUnits, checkSubmerge);
  }

  private static JBGBattleResult checkIfNoAttackersOrDefenders(
      final JBGData jbgData,
      final Territory t,
      final Collection<Unit> attackingUnits,
      final Collection<Unit> defendingUnits,
    ...
    if (attackingUnits.isEmpty() || (hasNoDefenders && isLandAndCanOnlyBeAttackedByAir)) {
      ...
      final double tuv = TuvUtils.getTuv(mainCombatDefenders, jbgData.getUnitValueMap());
      return new JBGBattleResult(100, 0.1 + tuv, true, attackingUnits, new ArrayList<>(), 0);
    } else if (canSubmergeBeforeBattle(
        jbgData.getData(), attackingUnits, defendingUnits, checkSubmerge)) {
      return new JBGBattleResult();
    }
    return null;
  }
  ...
  public JBGBattleResult callBattleCalcWithRetreatAir(
      final JBGData jbgData,
      final Territory t,
      final Collection<Unit> attackingUnits,
      final Collection<Unit> defendingUnits,
      final Collection<Unit> bombardingUnits) {
    return callBattleCalc(jbgData, t, attackingUnits, defendingUnits, bombardingUnits, true, true);
  }

  public JBGBattleResult callBattleCalc(
      final JBGData jbgData,
      final Territory t,
      final Collection<Unit> attackingUnits,
      final Collection<Unit> defendingUnits,
      final Collection<Unit> bombardingUnits) {
    return callBattleCalc(jbgData, t, attackingUnits, defendingUnits, bombardingUnits, true);
  }

  private JBGBattleResult callBattleCalc(
      final JBGData jbgData,
      final Territory t,
      final Collection<Unit> attackingUnits,
      final Collection<Unit> defendingUnits,
      ...
      final boolean checkSubmerge) {
    return callBattleCalc(
        jbgData, t, attackingUnits, defendingUnits, bombardingUnits, checkSubmerge, false);
  }
  ...
  private JBGBattleResult callBattleCalc(
    ...
    if (Matches.territoryIsNeutralButNotWater().test(t)) {
      final double attackingUnitValue =
          TuvUtils.getTuv(mainCombatAttackers, jbgData.getUnitValueMap());
      final double remainingUnitValue =
          results
              .getAverageTuvOfUnitsLeftOver(jbgData.getUnitValueMap(), jbgData.getUnitValueMap())
              .getFirst();
    ...
    if (t.isWater() && !defendingTransportedUnits.isEmpty()) {
      final double transportedUnitValue =
          TuvUtils.getTuv(defendingTransportedUnits, jbgData.getUnitValueMap());
    ...
    if (tuvSwing > 0
      ...
      final int subTuv = TuvUtils.getTuv(defendingSubsKilled, jbgData.getUnitValueMap());
