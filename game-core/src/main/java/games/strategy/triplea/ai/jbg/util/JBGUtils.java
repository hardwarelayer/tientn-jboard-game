...
/** JBG AI utilities (these are very general and maybe should be moved into delegate or engine). */
public final class JBGUtils {
  private JBGUtils() {}
  ...
  public static int getClosestEnemyLandTerritoryDistance(
    ...
    final Set<Territory> landTerritories =
        data.getMap()
            .getNeighbors(t, 9, JBGMatches.territoryCanPotentiallyMoveLandUnits(player, data));
      ...
      int distance =
          data.getMap()
              .getDistance(
                  ...
                  enemyLandTerritory,
                  JBGMatches.territoryCanPotentiallyMoveLandUnits(player, data));
      if (JBGUtils.isNeutralLand(enemyLandTerritory)) {
        distance++;
      }
      if (distance < minDistance) {
        minDistance = distance;
  ...
  public static int getClosestEnemyLandTerritoryDistanceOverWater(
    ...
    final List<Territory> enemyOrAdjacentLandTerritories =
        CollectionUtils.getMatches(
            neighborTerritories,
            JBGMatches.territoryIsOrAdjacentToEnemyNotNeutralLand(player, data));
    int minDistance = 10;
  ...
  public static boolean isNeutralLand(final Territory t) {
    return !t.isWater() && JBGUtils.isNeutralPlayer(t.getOwner());
  }
