...
/** JBG AI transport utilities. */
public final class JBGTransportUtils {
  private JBGTransportUtils() {}

  public static int findMaxMovementForTransports(
      final List<JBGPurchaseOption> seaTransportPurchaseOptions) {
    int maxMovement = 2;
    final int maxTransportEfficiency = 0;
    for (final JBGPurchaseOption ppo : seaTransportPurchaseOptions) {
  ...
  public static List<Unit> getUnitsToTransportFromTerritories(
    ...
    return getUnitsToTransportFromTerritories(
        ...
        unitsToIgnore,
        JBGMatches.unitIsOwnedTransportableUnitAndCanBeLoaded(player, transport, true));
  }
  ...
  public static List<Unit> getUnitsToAdd(
      final JBGData jbgData, final Unit unit, final Map<Territory, JBGTerritory> moveMap) {
    return getUnitsToAdd(jbgData, unit, new ArrayList<>(), moveMap);
  }
  ...
  public static List<Unit> getUnitsToAdd(
    ...
    return findBestUnitsToLandTransport(unit, jbgData.getUnitTerritory(unit), movedUnits);
  }

  public static List<Unit> getMovedUnits(
      final List<Unit> alreadyMovedUnits, final Map<Territory, JBGTerritory> attackMap) {
    final List<Unit> movedUnits = new ArrayList<>(alreadyMovedUnits);
    movedUnits.addAll(
        attackMap.values().stream()
            .map(JBGTerritory::getAllDefenders)
        ...
        t.getUnitCollection()
            .getMatches(
                Matches.unitIsOwnedBy(player)
                    ...
                    .and(JBGMatches.unitHasLessMovementThan(unit)));
  ...
  public static List<Unit> getAirThatCantLandOnCarrier(
    ...
    final Collection<Unit> airUnits =
        CollectionUtils.getMatches(units, JBGMatches.unitIsAlliedAir(player, data));
  ...
  public static boolean validateCarrierCapacity(
    ...
    final Collection<Unit> airUnits =
        CollectionUtils.getMatches(existingUnits, JBGMatches.unitIsAlliedAir(player, data));
  ...
  public static int getUnusedLocalCarrierCapacity(
    ...
    final Set<Territory> nearbyTerritories =
        data.getMap().getNeighbors(t, 2, JBGMatches.territoryCanMoveAirUnits(player, data, false));
    ...
    final Collection<Unit> airUnits =
        CollectionUtils.getMatches(ownedNearbyUnits, JBGMatches.unitIsOwnedAir(player));
    for (final Unit airUnit : airUnits) {
      final UnitAttachment ua = UnitAttachment.get(airUnit.getType());
      final int cost = ua.getCarrierCost();
      if (cost != -1) {
  ...
  public static int getUnusedCarrierCapacity(
    ...
    int capacity = AirMovementValidator.carrierCapacity(units, t);
    final Collection<Unit> airUnits =
        CollectionUtils.getMatches(units, JBGMatches.unitIsOwnedAir(player));
    for (final Unit airUnit : airUnits) {
      final UnitAttachment ua = UnitAttachment.get(airUnit.getType());
      final int cost = ua.getCarrierCost();
      if (cost != -1) {
