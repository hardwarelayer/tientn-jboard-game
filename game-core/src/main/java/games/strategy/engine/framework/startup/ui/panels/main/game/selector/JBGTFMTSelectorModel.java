...
//Load Tien's Format: article with kanjis from my python generator
//
/**
...
@Log
public class JBGTFMTSelectorModel extends Observable {

  private static final String TFMT_EXTENSION = ".tfmt";
  private static final String TFMT_SAVE_EXTENSION = ".twork";
  ...
  public List<JBGKanjiItem> load(final File file) throws Exception {
    ...
    //JBG
    System.out.println("JBGTFMTSelectorModel:Kanji File name is: "+file.getAbsolutePath());

    // if the file name is xml, load it as a new game
    if (file.getName().toLowerCase().endsWith(TFMT_EXTENSION)) {
      return loadKanjiFromResource(file);
    }
    else if (file.getName().toLowerCase().endsWith(TFMT_SAVE_EXTENSION)) {
